...
let projectDirectoryCache = new Map();
let lastCacheRefresh = 0;
const CACHE_TTL = 60000; // 1 minute
// Clear project directory cache
export const clearProjectDirectoryCache = () => {
    projectDirectoryCache.clear();
    lastCacheRefresh = 0;
};
// Extract project directory from Claude project name
export const extractProjectDirectory = async (projectName) => {
    const now = Date.now();
...
// Get all projects
export const getProjects = async (progressCallback) => {
    const projects = [];
    const projectsPath = CLAUDE_PROJECTS_PATH;
    if (!fs.existsSync(projectsPath)) {
        return projects;
...
export const getSessions = async (projectName, limit = 5, offset = 0) => {
    const sessions = [];
...
export const getSessionMessages = async (projectName, sessionId, limit, offset) => {
    const projectDir = await extractProjectDirectory(projectName);
    if (!projectDir) {
        return { messages: [] };
    }
    try {
        ...
        const messages = JSON.parse(fs.readFileSync(sessionPath, 'utf8'));
...
// Rename project
export const renameProject = async (projectName, displayName) => {
    const projectDir = await extractProjectDirectory(projectName);
    if (!projectDir) {
...
// Delete session
export const deleteSession = async (projectName, sessionId) => {
    const projectDir = await extractProjectDirectory(projectName);
    if (!projectDir) {
        throw new Error(`Project not found: ${projectName}`);
    }
...
// Delete project
export const deleteProject = async (projectName, force = false) => {
    const projectDir = await extractProjectDirectory(projectName);
    if (!projectDir) {
        throw new Error(`Project not found: ${projectName}`);
    }
...
export const addProjectManually = async (projectPath) => {
    try {
...
export const validateWorkspacePath = async (pathToValidate) => {
    try {
...
// Get file tree
export const getFileTree = async (dir, maxDepth = 10, currentDepth = 0, showHidden = false) => {
