 ...
 */
const processClaudeRequest = async (req, res, next) => {
    try {
        const { project, model, message } = req.body;
        if (!project || !message) {
 ...
 * @param next - Express next function
 */
const processCursorRequest = async (req, res, next) => {
    try {
        const { project, model, message } = req.body;
        if (!project || !message) {
            return res.status(400).json({ error: 'Project and message are required' });
 ...
 * @param next - Express next function
 */
const processCodexRequest = async (req, res, next) => {
    try {
        const { project, model, message } = req.body;
        if (!project || !message) {
            return res.status(400).json({ error: 'Project and message are required' });
 ...
 * @param next - Express next function
 */
const processPiRequest = async (req, res, next) => {
    try {
        const { project, model, message } = req.body;
        if (!project || !message) {
            return res.status(400).json({ error: 'Project and message are required' });
...
router.post('/request', validateExternalApiKey, async (req, res) => {
    try {
        const { project, model, message, provider } = req.body;
        if (!project || !message) {
            return res.status(400).json({ error: 'Project and message are required' });
        }
        // Determine which provider to use
        const selectedProvider = provider || req.user?.default_provider || 'claude';
...
router.get('/project/:name', async (req, res) => {
    try {
        ...
        const projectPath = await addProjectManually; // Placeholder
        if (!projectPath) {
...
// Get git remote URL
router.get('/git/remote/:project', async (req, res) => {
    try {
...
export { validateExternalApiKey, getGitRemoteUrl, getGitBranch, getGitStatus, processClaudeRequest, processCursorRequest, processCodexRequest, processPiRequest, };
