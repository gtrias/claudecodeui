...
function maskSensitiveValue(envVar) {
    return {
        ...envVar,
...
router.post('/global', async (req, res) => {
    try {
        const { key, value, is_sensitive = false } = req.body;
...
router.put('/global/:id', async (req, res) => {
    try {
        ...
        const success = environmentVariablesDb.updateGlobalEnvironmentVariable(parseInt(id), value, is_sensitive ?? false);
        if (success) {
...
router.put('/project/:projectId/:id', async (req, res) => {
    try {
        ...
        const success = environmentVariablesDb.updateProjectEnvironmentVariable(projectId, parseInt(id), value, is_sensitive ?? false);
        if (success) {
            res.json({ success: true });
        }
        else {
