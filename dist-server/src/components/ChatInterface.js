...
import { decodeHtmlEntities } from "../utils/chatUtils";
...
const markdownComponents = {
    code: CodeBlock,
    blockquote: ({ children }) => (_jsx("blockquote", { className: "border-l-4 border-gray-300 dark:border-gray-600 pl-4 italic text-gray-600 dark:text-gray-400 my-2", children: children })),
    a: ({ href, children }) => (_jsx("a", { href: href, className: "text-blue-600 dark:text-blue-400 hover:underline", target: "_blank", rel: "noopener noreferrer", children: children })),
    p: ({ children }) => _jsx("div", { className: "mb-2 last:mb-0", children: children }),
    // GFM tables
    table: ({ children }) => (_jsx("div", { className: "overflow-x-auto my-2", children: _jsx("table", { className: "min-w-full border-collapse border border-gray-200 dark:border-gray-700", children: children }) })),
    thead: ({ children }) => (_jsx("thead", { className: "bg-gray-50 dark:bg-gray-800", children: children })),
    th: ({ children }) => (_jsx("th", { className: "px-3 py-2 text-left text-sm font-semibold border border-gray-200 dark:border-gray-700", children: children })),
    td: ({ children }) => (_jsx("td", { className: "px-3 py-2 align-top text-sm border border-gray-200 dark:border-gray-700", children: children }))
};
...
const MessageComponent = memo(({ message, index, prevMessage, createDiff, onFileOpen, onShowSettings, onGrantToolPermission, autoExpandTools, showRawParameters, showThinking, selectedProject, provider }) => {
    ...
    return (_jsx("div", { ref: messageRef, className: `chat-message ${message.type} ${isGrouped ? 'grouped' : ''} ${message.type === 'user' ? 'flex justify-end px-3 sm:px-0' : 'px-3 sm:px-0'}`, children: message.type === 'user' ? (
        /* User message bubble on the right */
        _jsxs("div", { className: "flex items-end space-x-0 sm:space-x-3 w-full sm:w-auto sm:max-w-[85%] md:max-w-md lg:max-w-lg xl:max-w-xl", children: [_jsxs("div", { className: "bg-blue-600 text-white rounded-2xl rounded-br-md px-3 sm:px-4 py-2 shadow-sm flex-1 sm:flex-initial", children: [_jsx("div", { className: "text-sm whitespace-pre-wrap break-words", children: message.content }), message.images && message.images.length > 0 && (_jsx("div", { className: "mt-2 grid grid-cols-2 gap-2", children: message.images.map((img, idx) => (_jsx("img", { src: img.data, alt: img.name, className: "rounded-lg max-w-full h-auto cursor-pointer hover:opacity-90 transition-opacity", onClick: () => window.open(img.data, '_blank') }, idx))) })), _jsx("div", { className: "text-xs text-blue-100 mt-1 text-right", children: new Date(message.timestamp).toLocaleTimeString() })] }), !isGrouped && (_jsx("div", { className: "hidden sm:flex w-8 h-8 bg-blue-600 rounded-full items-center justify-center text-white text-sm flex-shrink-0", children: "U" }))] })) : (
        /* Claude/Error/Tool messages on the left */
        _jsxs("div", { className: "w-full", children: [!isGrouped && (_jsxs("div", { className: "flex items-center space-x-3 mb-2", children: [message.type === 'error' ? (_jsx("div", { className: "w-8 h-8 bg-red-600 rounded-full flex items-center justify-center text-white text-sm flex-shrink-0", children: "!" })) : message.type === 'tool' ? (_jsx("div", { className: "w-8 h-8 bg-gray-600 dark:bg-gray-700 rounded-full flex items-center justify-center text-white text-sm flex-shrink-0", children: "\uD83D\uDD27" })) : (_jsx("div", { className: "w-8 h-8 rounded-full flex items-center justify-center text-white text-sm flex-shrink-0 p-1", children: (localStorage.getItem('selected-provider') || 'claude') === 'cursor' ? (_jsx(CursorLogo, { className: "w-full h-full" })) : (localStorage.getItem('selected-provider') || 'claude') === 'codex' ? (_jsx(CodexLogo, { className: "w-full h-full" })) : (localStorage.getItem('selected-provider') || 'claude') === 'pi' ? (_jsx(PiLogo, { className: "w-full h-full" })) : (_jsx(ClaudeLogo, { className: "w-full h-full" })) })), _jsx("div", { className: "text-sm font-medium text-gray-900 dark:text-white", children: message.type === 'error' ? t('messageTypes.error') : message.type === 'tool' ? t('messageTypes.tool') : ((localStorage.getItem('selected-provider') || 'claude') === 'cursor' ? t('messageTypes.cursor') : (localStorage.getItem('selected-provider') || 'claude') === 'codex' ? t('messageTypes.codex') : (localStorage.getItem('selected-provider') || 'claude') === 'pi' ? t('messageTypes.pi') : t('messageTypes.claude')) })] })), _jsxs("div", { className: "w-full", children: [message.isToolUse && !['Read', 'TodoWrite', 'TodoRead'].includes(message.toolName) ? ((() => {
                            // Minimize Grep and Glob tools since they happen frequently
                            const isSearchTool = ['Grep', 'Glob'].includes(message.toolName);
                            if (isSearchTool) {
                                return (_jsx(_Fragment, { children: _jsx("div", { className: "group relative bg-gray-50/50 dark:bg-gray-800/30 border-l-2 border-blue-400 dark:border-blue-500 pl-3 py-2 my-2", children: _jsxs("div", { className: "flex items-center justify-between gap-3", children: [_jsxs("div", { className: "flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 flex-1 min-w-0", children: [_jsx("svg", { className: "w-3.5 h-3.5 text-blue-500 dark:text-blue-400 flex-shrink-0", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" }) }), _jsx("span", { className: "font-medium flex-shrink-0", children: message.toolName }), _jsx("span", { className: "text-gray-400 dark:text-gray-500 flex-shrink-0", children: "\u2022" }), message.toolInput && (() => {
                                                            try {
                                                                const input = JSON.parse(message.toolInput);
                                                                return (_jsxs("span", { className: "font-mono truncate flex-1 min-w-0", children: [input.pattern && _jsxs("span", { children: [t('search.pattern'), " ", _jsx("span", { className: "text-blue-600 dark:text-blue-400", children: input.pattern })] }), input.path && _jsxs("span", { className: "ml-2", children: [t('search.in'), " ", input.path] })] }));
                                                            }
                                                            catch (e) {
                                                                return null;
                                                            }
                                                        })()] }), message.toolResult && (_jsxs("a", { href: `#tool-result-${message.toolId}`, className: "flex-shrink-0 text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium transition-colors flex items-center gap-1", children: [_jsx("span", { children: t('tools.searchResults') }), _jsx("svg", { className: "w-3 h-3", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) })] }))] }) }) }));
                            }
                            // Full display for other tools
                            return (_jsxs("div", { className: "group relative bg-gradient-to-br from-blue-50/50 to-indigo-50/50 dark:from-blue-950/20 dark:to-indigo-950/20 border border-blue-100/30 dark:border-blue-800/30 rounded-lg p-3 mb-2", children: [_jsx("div", { className: "absolute inset-0 bg-gradient-to-br from-blue-500/3 to-indigo-500/3 dark:from-blue-400/3 dark:to-indigo-400/3 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" }), _jsxs("div", { className: "relative flex items-center justify-between mb-3", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsxs("div", { className: "relative w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-600 dark:from-blue-400 dark:to-indigo-500 rounded-lg flex items-center justify-center shadow-lg shadow-blue-500/20 dark:shadow-blue-400/20", children: [_jsxs("svg", { className: "w-4 h-4 text-white", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: [_jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" }), _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M15 12a3 3 0 11-6 0 3 3 0 016 0z" })] }), _jsx("div", { className: "absolute inset-0 rounded-lg bg-blue-500 dark:bg-blue-400 animate-pulse opacity-20" })] }), _jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: "font-semibold text-gray-900 dark:text-white text-sm", children: message.toolName }), _jsx("span", { className: "text-xs text-gray-500 dark:text-gray-400 font-mono", children: message.toolId })] })] }), onShowSettings && (_jsx("button", { onClick: (e) => {
                                                    e.stopPropagation();
                                                    onShowSettings();
                                                }, className: "p-2 rounded-lg hover:bg-white/60 dark:hover:bg-gray-800/60 transition-all duration-200 group/btn backdrop-blur-sm", title: t('tools.settings'), children: _jsxs("svg", { className: "w-4 h-4 text-gray-600 dark:text-gray-400 group-hover/btn:text-blue-600 dark:group-hover/btn:text-blue-400 transition-colors", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: [_jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" }), _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M15 12a3 3 0 11-6 0 3 3 0 016 0z" })] }) }))] }), message.toolInput && message.toolName === 'Edit' && (() => {
                                        try {
                                            const input = JSON.parse(message.toolInput);
                                            if (input.file_path && input.old_string && input.new_string) {
                                                return (_jsxs("details", { className: "relative mt-3 group/details", open: autoExpandTools, children: [_jsxs("summary", { className: "flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 p-2.5 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50", children: [_jsx("svg", { className: "w-4 h-4 transition-transform duration-200 group-open/details:rotate-180", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), _jsx("span", { className: "flex items-center gap-2", children: _jsx("span", { children: "View edit diff for" }) }), _jsx("button", { onClick: async (e) => {
                                                                        e.preventDefault();
                                                                        e.stopPropagation();
                                                                        if (!onFileOpen)
                                                                            return;
                                                                        try {
                                                                            ...
                                                                            onFileOpen(input.file_path);
                                                                    ...
                                                                    }, className: "px-2.5 py-1 rounded-md bg-white/60 dark:bg-gray-800/60 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 font-mono text-xs font-medium transition-all duration-200 shadow-sm", children: input.file_path ? input.file_path.split('/').pop() : '' })] }), _jsxs("div", { className: "mt-3 pl-6", children: [_jsxs("div", { className: "bg-white dark:bg-gray-900/50 border border-gray-200/60 dark:border-gray-700/60 rounded-lg overflow-hidden shadow-sm", children: [_jsxs("div", { className: "flex items-center justify-between px-4 py-2.5 bg-gradient-to-r from-gray-50 to-gray-100/50 dark:from-gray-800/80 dark:to-gray-800/40 border-b border-gray-200/60 dark:border-gray-700/60 backdrop-blur-sm", children: [_jsx("button", { onClick: async () => {
                                                                                        if (!onFileOpen)
                                                                                            return;
                                                                                        try {
                                                                                            // Fetch the current file (after the edit)
                                                                                    ...
                                                                                    }, className: "text-xs font-mono text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 truncate cursor-pointer font-medium transition-colors", children: input.file_path }), _jsx("span", { className: "text-xs text-gray-500 dark:text-gray-400 font-medium px-2 py-0.5 bg-gray-100 dark:bg-gray-700/50 rounded", children: "Diff" })] }), _jsx("div", { className: "text-xs font-mono", children: createDiff(input.old_string, input.new_string).map((diffLine, i) => (_jsxs("div", { className: "flex", children: [_jsx("span", { className: `w-8 text-center border-r ${diffLine.type === 'removed'
                                                                                            ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800'
                                                                                            : 'bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border-green-200 dark:border-green-800'}`, children: diffLine.type === 'removed' ? '-' : '+' }), _jsx("span", { className: `px-2 py-0.5 flex-1 whitespace-pre-wrap ${diffLine.type === 'removed'
                                                                                            ? 'bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-200'
                                                                                            : 'bg-green-50 dark:bg-green-900/20 text-green-800 dark:text-green-200'}`, children: diffLine.content })] }, i))) })] }), showRawParameters && (_jsxs("details", { className: "relative mt-3 pl-6 group/raw", open: autoExpandTools, children: [_jsxs("summary", { className: "flex items-center gap-2 text-xs font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 p-2 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50", children: [_jsx("svg", { className: "w-3 h-3 transition-transform duration-200 group-open/raw:rotate-180", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), "View raw parameters"] }), _jsx("pre", { className: "mt-2 text-xs bg-gray-50 dark:bg-gray-800/50 border border-gray-200/60 dark:border-gray-700/60 p-3 rounded-lg whitespace-pre-wrap break-words overflow-hidden text-gray-700 dark:text-gray-300 font-mono", children: message.toolInput })] }))] })] }));
                                            }
                                        }
                                        catch (e) {
                                            // Fall back to raw display if parsing fails
                                        }
                                        return (_jsxs("details", { className: "relative mt-3 group/params", open: autoExpandTools, children: [_jsxs("summary", { className: "flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 p-2.5 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50", children: [_jsx("svg", { className: "w-4 h-4 transition-transform duration-200 group-open/params:rotate-180", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), "View input parameters"] }), _jsx("pre", { className: "mt-3 text-xs bg-gray-50 dark:bg-gray-800/50 border border-gray-200/60 dark:border-gray-700/60 p-3 rounded-lg whitespace-pre-wrap break-words overflow-hidden text-gray-700 dark:text-gray-300 font-mono", children: message.toolInput })] }));
                                        ...
                                        if (message.toolName === 'Write') {
                                            try {
                                                ...
                                                if (input.file_path && input.content !== undefined) {
                                                    return (_jsxs("details", { className: "relative mt-3 group/details", open: autoExpandTools, children: [_jsxs("summary", { className: "flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 p-2.5 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50", children: [_jsx("svg", { className: "w-4 h-4 transition-transform duration-200 group-open/details:rotate-180", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), _jsxs("span", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-lg leading-none", children: "\uD83D\uDCC4" }), _jsx("span", { children: "Creating new file:" })] }), _jsx("button", { onClick: async (e) => {
                                                                            e.preventDefault();
                                                                            e.stopPropagation();
                                                                            if (!onFileOpen)
                                                                                return;
                                                                            try {
                                                                                ...
                                                                                onFileOpen(input.file_path, {
                                                                                    ...
                                                                                    new_string: input.content || ''
                                                                                        ...
                                                                                        }, className: "text-xs font-mono text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 truncate cursor-pointer font-medium transition-colors", children: input.file_path }), _jsx("span", { className: "text-xs text-gray-500 dark:text-gray-400 font-medium px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded", children: "New File" })] }), _jsx("div", { className: "text-xs font-mono", children: createDiff('', input.content).map((diffLine, i) => (_jsxs("div", { className: "flex", children: [_jsx("span", { className: `w-8 text-center border-r ${diffLine.type === 'removed'
                                                                                                ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800'
                                                                                                : 'bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border-green-200 dark:border-green-800'}`, children: diffLine.type === 'removed' ? '-' : '+' }), _jsx("span", { className: `px-2 py-0.5 flex-1 whitespace-pre-wrap ${diffLine.type === 'removed'
                                                                                                ? 'bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-200'
                                                                                                : 'bg-green-50 dark:bg-green-900/20 text-green-800 dark:text-green-200'}`, children: diffLine.content })] }, i))) })] }), showRawParameters && (_jsxs("details", { className: "relative mt-3 pl-6 group/raw", open: autoExpandTools, children: [_jsxs("summary", { className: "flex items-center gap-2 text-xs font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 p-2 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50", children: [_jsx("svg", { className: "w-3 h-3 transition-transform duration-200 group-open/raw:rotate-180", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), "View raw parameters"] }), _jsx("pre", { className: "mt-2 text-xs bg-gray-50 dark:bg-gray-800/50 border border-gray-200/60 dark:border-gray-700/60 p-3 rounded-lg whitespace-pre-wrap break-words overflow-hidden text-gray-700 dark:text-gray-300 font-mono", children: message.toolInput })] }))] })] }));
                                        ...
                                        if (message.toolName === 'TodoWrite') {
                                            try {
                                                ...
                                                if (input.todos && Array.isArray(input.todos)) {
                                                    return (_jsxs("details", { className: "relative mt-3 group/todo", open: autoExpandTools, children: [_jsxs("summary", { className: "flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 p-2.5 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50", children: [_jsx("svg", { className: "w-4 h-4 transition-transform duration-200 group-open/todo:rotate-180", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), _jsxs("span", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-lg leading-none", children: "\u2713" }), _jsx("span", { children: "Updating Todo List" })] })] }), _jsxs("div", { className: "mt-3", children: [_jsx(TodoList, { todos: input.todos }), showRawParameters && (_jsxs("details", { className: "relative mt-3 group/raw", open: autoExpandTools, children: [_jsxs("summary", { className: "flex items-center gap-2 text-xs font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 p-2 rounded-lg hover:bg-white/50 dark:hover:bg-gray-800/50", children: [_jsx("svg", { className: "w-3 h-3 transition-transform duration-200 group-open/raw:rotate-180", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), "View raw parameters"] }), _jsx("pre", { className: "mt-2 text-xs bg-gray-50 dark:bg-gray-800/50 border border-gray-200/60 dark:border-gray-700/60 p-3 rounded-lg overflow-x-auto text-gray-700 dark:text-gray-300 font-mono", children: message.toolInput })] }))] })] }));
                                                }
                                            }
                                            catch (e) {
                                                // Fall back to regular display
                                        ...
                                        if (message.toolName === 'Bash') {
                                            try {
                                                const input = JSON.parse(message.toolInput);
                                                return (_jsxs("div", { className: "my-2", children: [_jsxs("div", { className: "bg-gray-900 dark:bg-gray-950 rounded-md px-3 py-2 font-mono text-sm", children: [_jsx("span", { className: "text-green-400", children: "$" }), _jsx("span", { className: "text-gray-100 ml-2", children: input.command })] }), input.description && (_jsx("div", { className: "mt-1 text-xs text-gray-500 dark:text-gray-400 italic ml-1", children: input.description }))] }));
                                            }
                                            catch (e) {
                                                // Fall back to regular display
                                            }
                                        ...
                                        if (message.toolName === 'Read') {
                                            try {
                                                ...
                                                if (input.file_path) {
                                                    ...
                                                    return (_jsxs("div", { className: "mt-2 text-sm text-blue-700 dark:text-blue-300", children: ["Read", ' ', _jsx("button", { onClick: () => onFileOpen && onFileOpen(input.file_path), className: "text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 underline font-mono", children: filename })] }));
                                                }
                                            }
                                            catch (e) {
                                                // Fall back to regular display
                                        ...
                                        if (message.toolName === 'exit_plan_mode') {
                                            try {
                                                ...
                                                if (input.plan) {
                                                    ...
                                                    return (_jsxs("details", { className: "mt-2", open: autoExpandTools, children: [_jsxs("summary", { className: "text-sm text-blue-700 dark:text-blue-300 cursor-pointer hover:text-blue-800 dark:hover:text-blue-200 flex items-center gap-2", children: [_jsx("svg", { className: "w-4 h-4 transition-transform details-chevron", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), "\uD83D\uDCCB View implementation plan"] }), _jsx("div", { className: "mt-3 prose prose-sm max-w-none dark:prose-invert", children: _jsx(ReactMarkdown, { children: planContent }) })] }));
                                                }
                                            }
                                            catch (e) {
                                                // Fall back to regular display
                                        ...
                                        return (_jsxs("div", { id: `tool-result-${message.toolId}`, className: `relative mt-4 p-4 rounded-lg border backdrop-blur-sm scroll-mt-4 ${message.toolResult.isError
                                                ...
                                                : 'bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 border-green-200/60 dark:border-green-800/60'}`, children: [_jsx("div", { className: `absolute inset-0 rounded-lg opacity-50 ${message.toolResult.isError
                                                        ? 'bg-gradient-to-br from-red-500/5 to-rose-500/5 dark:from-red-400/5 dark:to-rose-400/5'
                                                        : 'bg-gradient-to-br from-green-500/5 to-emerald-500/5 dark:from-green-400/5 dark:to-emerald-400/5'}` }), _jsxs("div", { className: "relative flex items-center gap-2.5 mb-3", children: [_jsx("div", { className: `w-6 h-6 rounded-lg flex items-center justify-center shadow-md ${message.toolResult.isError
                                                                ? 'bg-gradient-to-br from-red-500 to-rose-600 dark:from-red-400 dark:to-rose-500 shadow-red-500/20'
                                                                : 'bg-gradient-to-br from-green-500 to-emerald-600 dark:from-green-400 dark:to-emerald-500 shadow-green-500/20'}`, children: _jsx("svg", { className: "w-3.5 h-3.5 text-white", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: message.toolResult.isError ? (_jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2.5, d: "M6 18L18 6M6 6l12 12" })) : (_jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2.5, d: "M5 13l4 4L19 7" })) }) }), _jsx("span", { className: `text-sm font-semibold ${message.toolResult.isError
                                                                ? 'text-red-800 dark:text-red-200'
                                                                : 'text-green-800 dark:text-green-200'}`, children: message.toolResult.isError ? 'Tool Error' : 'Tool Result' })] }), _jsxs("div", { className: `relative text-sm ${message.toolResult.isError
                                                        ? 'text-red-900 dark:text-red-100'
                                                        : 'text-green-900 dark:text-green-100'}`, children: [(() => {
                                                            ...
                                                            if ((message.toolName === 'TodoWrite' || message.toolName === 'TodoRead') &&
                                                                ...
                                                                try {
                                                                    ...
                                                                    if (content.startsWith('[')) {
                                                                        ...
                                                                        return (_jsx("div", { children: _jsx("div", { className: "flex items-center gap-2 mb-2", children: _jsx("span", { className: "font-medium", children: "Todo list has been updated successfully" }) }) }));
                                                                    }
                                                                    if (todos && Array.isArray(todos)) {
                                                                        return (_jsxs("div", { children: [_jsx("div", { className: "flex items-center gap-2 mb-3", children: _jsx("span", { className: "font-medium", children: "Current Todo List" }) }), _jsx(TodoList, { todos: todos, isResult: true })] }));
                                                                    }
                                                                }
                                                                catch (e) {
                                                            ...
                                                            if (message.toolName === 'exit_plan_mode') {
                                                                try {
                                                                    ...
                                                                    if (parsed.plan) {
                                                                        ...
                                                                        return (_jsxs("div", { children: [_jsx("div", { className: "flex items-center gap-2 mb-3", children: _jsx("span", { className: "font-medium", children: "Implementation Plan" }) }), _jsx("div", { className: "prose prose-sm max-w-none dark:prose-invert", children: _jsx(ReactMarkdown, { children: planContent }) })] }));
                                                                    }
                                                                }
                                                                catch (e) {
                                                                    // Fall through to regular handling
                                                            ...
                                                            if ((message.toolName === 'Grep' || message.toolName === 'Glob') && message.toolResult?.toolUseResult) {
                                                                ...
                                                                if (toolData.filenames && Array.isArray(toolData.filenames) && toolData.filenames.length > 0) {
                                                                    return (_jsxs("div", { children: [_jsx("div", { className: "flex items-center gap-2 mb-3", children: _jsxs("span", { className: "font-medium", children: ["Found ", toolData.numFiles || toolData.filenames.length, " ", (toolData.numFiles === 1 || toolData.filenames.length === 1) ? 'file' : 'files'] }) }), _jsx("div", { className: "space-y-1 max-h-96 overflow-y-auto", children: toolData.filenames.map((filePath, index) => {
                                                                                    ...
                                                                                    return (_jsxs("div", { onClick: () => {
                                                                                        ...
                                                                                        }, className: "group flex items-center gap-2 px-2 py-1.5 rounded hover:bg-green-100/50 dark:hover:bg-green-800/20 cursor-pointer transition-colors", children: [_jsx("svg", { className: "w-4 h-4 text-green-600 dark:text-green-400 flex-shrink-0", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" }) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("div", { className: "font-mono text-sm font-medium text-green-800 dark:text-green-200 truncate group-hover:text-green-900 dark:group-hover:text-green-100", children: fileName }), _jsx("div", { className: "font-mono text-xs text-green-600/70 dark:text-green-400/70 truncate", children: dirPath })] }), _jsx("svg", { className: "w-4 h-4 text-green-600 dark:text-green-400 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 5l7 7-7 7" }) })] }, index));
                                                                                }) })] }));
                                                                }
                                                            }
                                                            ...
                                                            if (content.includes('Do you want to proceed?') && message.toolName === 'Bash') {
                                                                ...
                                                                return (_jsxs("div", { className: "space-y-3", children: [beforePrompt && (_jsx("div", { className: "bg-gray-900 dark:bg-gray-950 text-gray-100 rounded-lg p-3 font-mono text-xs overflow-x-auto", children: _jsx("pre", { className: "whitespace-pre-wrap break-words", children: beforePrompt }) })), _jsx("div", { className: "bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4", children: _jsxs("div", { className: "flex items-start gap-3", children: [_jsx("div", { className: "w-8 h-8 bg-amber-500 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5", children: _jsx("svg", { className: "w-5 h-5 text-white", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" }) }) }), _jsxs("div", { className: "flex-1", children: [_jsx("h4", { className: "font-semibold text-amber-900 dark:text-amber-100 text-base mb-2", children: "Interactive Prompt" }), _jsx("p", { className: "text-sm text-amber-800 dark:text-amber-200 mb-4", children: questionLine }), _jsx("div", { className: "space-y-2 mb-4", children: options.map((option) => (_jsx("button", { className: `w-full text-left px-4 py-3 rounded-lg border-2 transition-all ${selectedOption === option.number
                                                                                                        ? 'bg-amber-600 dark:bg-amber-700 text-white border-amber-600 dark:border-amber-700 shadow-md'
                                                                                                        : 'bg-white dark:bg-gray-800 text-amber-900 dark:text-amber-100 border-amber-300 dark:border-amber-700 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-sm'} ${selectedOption ? 'cursor-default' : 'cursor-not-allowed opacity-75'}`, disabled: true, children: _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("span", { className: `flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${selectedOption === option.number
                                                                                                                    ? 'bg-white/20'
                                                                                                                    : 'bg-amber-100 dark:bg-amber-800/50'}`, children: option.number }), _jsx("span", { className: "text-sm sm:text-base font-medium flex-1", children: option.text }), selectedOption === option.number && (_jsx("svg", { className: "w-5 h-5 flex-shrink-0", fill: "currentColor", viewBox: "0 0 20 20", children: _jsx("path", { fillRule: "evenodd", d: "M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z", clipRule: "evenodd" }) }))] }) }, option.number))) }), selectedOption && (_jsxs("div", { className: "bg-amber-100 dark:bg-amber-800/30 rounded-lg p-3", children: [_jsxs("p", { className: "text-amber-900 dark:text-amber-100 text-sm font-medium mb-1", children: ["\u2713 Claude selected option ", selectedOption] }), _jsx("p", { className: "text-amber-800 dark:text-amber-200 text-xs", children: "In the CLI, you would select this option interactively using arrow keys or by typing the number." })] }))] })] }) })] }));
                                                            }
                                                            const fileEditMatch = content.match(/The file (.+?) has been updated\./);
                                                            if (fileEditMatch) {
                                                                return (_jsxs("div", { children: [_jsx("div", { className: "flex items-center gap-2 mb-2", children: _jsx("span", { className: "font-medium", children: "File updated successfully" }) }), _jsx("button", { onClick: async () => {
                                                                                if (!onFileOpen)
                                                                                    return;
                                                                            ...
                                                                            }, className: "text-xs font-mono bg-green-100 dark:bg-green-800/30 px-2 py-1 rounded text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 underline cursor-pointer", children: fileEditMatch[1] })] }));
                                                            }
                                                            ...
                                                            if (fileCreateMatch) {
                                                                return (_jsxs("div", { children: [_jsx("div", { className: "flex items-center gap-2 mb-2", children: _jsx("span", { className: "font-medium", children: "File created successfully" }) }), _jsx("button", { onClick: async () => {
                                                                                if (!onFileOpen)
                                                                                    return;
                                                                                // Fetch FULL file content with diff from git
                                                                                try {
                                                                            ...
                                                                            }, className: "text-xs font-mono bg-green-100 dark:bg-green-800/30 px-2 py-1 rounded text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 underline cursor-pointer", children: fileCreateMatch[1] })] }));
                                                            }
                                                            ...
                                                            if (message.toolName === 'Write' && !message.toolResult.isError) {
                                                                ...
                                                                return (_jsxs("div", { className: "text-green-700 dark:text-green-300", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("svg", { className: "w-4 h-4", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" }) }), _jsx("span", { className: "font-medium", children: "File written successfully" })] }), _jsx("p", { className: "text-xs mt-1 text-green-600 dark:text-green-400", children: "The file content is displayed in the diff view above" })] }));
                                                            }
                                                            if (content.includes('cat -n') && content.includes('→')) {
                                                                return (_jsxs("details", { open: autoExpandTools, children: [_jsxs("summary", { className: "text-sm text-green-700 dark:text-green-300 cursor-pointer hover:text-green-800 dark:hover:text-green-200 mb-2 flex items-center gap-2", children: [_jsx("svg", { className: "w-4 h-4 transition-transform details-chevron", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), "View file content"] }), _jsx("div", { className: "mt-2 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden", children: _jsx("div", { className: "text-xs font-mono p-3 whitespace-pre-wrap break-words overflow-hidden", children: content }) })] }));
                                                            }
                                                            if (content.length > 300) {
                                                                return (_jsxs("details", { open: autoExpandTools, children: [_jsxs("summary", { className: "text-sm text-green-700 dark:text-green-300 cursor-pointer hover:text-green-800 dark:hover:text-green-200 mb-2 flex items-center gap-2", children: [_jsx("svg", { className: "w-4 h-4 transition-transform details-chevron", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 9l-7 7-7-7" }) }), "View full output (", content.length, " chars)"] }), _jsx("div", { className: "mt-2 prose prose-sm max-w-none prose-green dark:prose-invert", children: _jsx(ReactMarkdown, { children: content }) })] }));
                                                            }
                                                            return (_jsx("div", { className: "prose prose-sm max-w-none prose-green dark:prose-invert", children: _jsx(ReactMarkdown, { children: content }) }));
                                                        })(), permissionSuggestion && (_jsxs("div", { className: "mt-4 border-t border-red-200/60 dark:border-red-800/60 pt-3", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-2", children: [_jsx("button", { type: "button", onClick: () => {
                                                                                if (!onGrantToolPermission)
                                                                                    return;
                                                                            ...
                                                                            }, disabled: permissionSuggestion.isAllowed || permissionGrantState === 'granted', className: `inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium border transition-colors ${permissionSuggestion.isAllowed || permissionGrantState === 'granted'
                                                                                ...
                                                                                : 'bg-white/80 dark:bg-gray-900/40 border-red-300/70 dark:border-red-800/60 text-red-700 dark:text-red-200 hover:bg-white dark:hover:bg-gray-900/70'}`, children: permissionSuggestion.isAllowed || permissionGrantState === 'granted'
                                                                                ? 'Permission added'
                                                                                : `Grant permission for ${permissionSuggestion.toolName}` }), onShowSettings && (_jsx("button", { type: "button", onClick: (e) => {
                                                                                e.stopPropagation();
                                                                                onShowSettings();
                                                                            }, className: "text-xs text-red-700 dark:text-red-200 underline hover:text-red-800 dark:hover:text-red-100", children: "Open settings" }))] }), _jsxs("div", { className: "mt-2 text-xs text-red-700/90 dark:text-red-200/80", children: ["Adds ", _jsx("span", { className: "font-mono", children: permissionSuggestion.entry }), " to Allowed Tools."] }), permissionGrantState === 'error' && (_jsx("div", { className: "mt-2 text-xs text-red-700 dark:text-red-200", children: "Unable to update permissions. Please try again." })), (permissionSuggestion.isAllowed || permissionGrantState === 'granted') && (_jsx("div", { className: "mt-2 text-xs text-green-700 dark:text-green-200", children: "Permission saved. Retry the request to use the tool." }))] }))] })] }));
                                    })()] }));
                        })()) : message.isInteractivePrompt ? (
                        // Special handling for interactive prompts
                        _jsx("div", { className: "bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4", children: _jsxs("div", { className: "flex items-start gap-3", children: [_jsx("div", { className: "w-8 h-8 bg-amber-500 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5", children: _jsx("svg", { className: "w-5 h-5 text-white", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" }) }) }), _jsxs("div", { className: "flex-1", children: [_jsx("h4", { className: "font-semibold text-amber-900 dark:text-amber-100 text-base mb-3", children: "Interactive Prompt" }), (() => {
                                                ...
                                                return (_jsxs(_Fragment, { children: [_jsx("p", { className: "text-sm text-amber-800 dark:text-amber-200 mb-4", children: questionLine }), _jsx("div", { className: "space-y-2 mb-4", children: options.map((option) => (_jsx("button", { className: `w-full text-left px-4 py-3 rounded-lg border-2 transition-all ${option.isSelected
                                                                    ? 'bg-amber-600 dark:bg-amber-700 text-white border-amber-600 dark:border-amber-700 shadow-md'
                                                                    : 'bg-white dark:bg-gray-800 text-amber-900 dark:text-amber-100 border-amber-300 dark:border-amber-700'} cursor-not-allowed opacity-75`, disabled: true, children: _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("span", { className: `flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${option.isSelected
                                                                                ? 'bg-white/20'
                                                                                : 'bg-amber-100 dark:bg-amber-800/50'}`, children: option.number }), _jsx("span", { className: "text-sm sm:text-base font-medium flex-1", children: option.text }), option.isSelected && (_jsx("span", { className: "text-lg", children: "\u276F" }))] }) }, option.number))) }), _jsxs("div", { className: "bg-amber-100 dark:bg-amber-800/30 rounded-lg p-3", children: [_jsx("p", { className: "text-amber-900 dark:text-amber-100 text-sm font-medium mb-1", children: "\u23F3 Waiting for your response in the CLI" }), _jsx("p", { className: "text-amber-800 dark:text-amber-200 text-xs", children: "Please select an option in your terminal where Claude is running." })] })] }));
                        ...
                        (() => {
                            try {
                                ...
                                if (input.file_path) {
                                    const filename = input.file_path.split('/').pop() || '';
                                    return (_jsx("div", { className: "bg-gray-50/50 dark:bg-gray-800/30 border-l-2 border-gray-400 dark:border-gray-500 pl-3 py-2 my-2", children: _jsxs("div", { className: "flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400", children: [_jsx("svg", { className: "w-3.5 h-3.5 text-gray-500 dark:text-gray-400", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" }) }), _jsx("span", { className: "font-medium", children: "Read" }), _jsx("button", { onClick: () => onFileOpen && onFileOpen(input.file_path), className: "text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-mono transition-colors", children: filename })] }) }));
                                }
                            }
                            catch (e) {
                                return (_jsx("div", { className: "bg-gray-50/50 dark:bg-gray-800/30 border-l-2 border-gray-400 dark:border-gray-500 pl-3 py-2 my-2", children: _jsxs("div", { className: "flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400", children: [_jsx("svg", { className: "w-3.5 h-3.5 text-gray-500 dark:text-gray-400", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" }) }), _jsx("span", { className: "font-medium", children: "Read file" })] }) }));
                            }
                        ...
                        (() => {
                            try {
                                const input = JSON.parse(message.toolInput);
                                if (input.todos && Array.isArray(input.todos)) {
                                    return (_jsxs("div", { className: "bg-gray-50/50 dark:bg-gray-800/30 border-l-2 border-gray-400 dark:border-gray-500 pl-3 py-2 my-2", children: [_jsxs("div", { className: "flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 mb-2", children: [_jsx("svg", { className: "w-3.5 h-3.5 text-gray-500 dark:text-gray-400", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" }) }), _jsx("span", { className: "font-medium", children: "Update todo list" })] }), _jsx(TodoList, { todos: input.todos })] }));
                                }
                            }
                            catch (e) {
                                return (_jsx("div", { className: "bg-gray-50/50 dark:bg-gray-800/30 border-l-2 border-gray-400 dark:border-gray-500 pl-3 py-2 my-2", children: _jsxs("div", { className: "flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400", children: [_jsx("svg", { className: "w-3.5 h-3.5 text-gray-500 dark:text-gray-400", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" }) }), _jsx("span", { className: "font-medium", children: "Update todo list" })] }) }));
                            }
                        })()) : message.isToolUse && message.toolName === 'TodoRead' ? (
                        // Simple TodoRead tool indicator
                        _jsx("div", { className: "bg-gray-50/50 dark:bg-gray-800/30 border-l-2 border-gray-400 dark:border-gray-500 pl-3 py-2 my-2", children: _jsxs("div", { className: "flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400", children: [_jsx("svg", { className: "w-3.5 h-3.5 text-gray-500 dark:text-gray-400", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" }) }), _jsx("span", { className: "font-medium", children: "Read todo list" })] }) })) : message.isThinking ? (
                        /* Thinking messages - collapsible by default */
                        _jsx("div", { className: "text-sm text-gray-700 dark:text-gray-300", children: _jsxs("details", { className: "group", children: [_jsxs("summary", { className: "cursor-pointer text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 font-medium flex items-center gap-2", children: [_jsx("svg", { className: "w-3 h-3 transition-transform group-open:rotate-90", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 5l7 7-7 7" }) }), _jsx("span", { children: "\uD83D\uDCAD Thinking..." })] }), _jsx("div", { className: "mt-2 pl-4 border-l-2 border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 text-sm", children: _jsx("div", { className: "prose prose-sm max-w-none dark:prose-invert prose-gray", children: _jsx(ReactMarkdown, { children: message.content }) }) })] }) })) : (_jsxs("div", { className: "text-sm text-gray-700 dark:text-gray-300", children: [showThinking && message.reasoning && (_jsxs("details", { className: "mb-3", children: [_jsx("summary", { className: "cursor-pointer text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 font-medium", children: "\uD83D\uDCAD Thinking..." }), _jsx("div", { className: "mt-2 pl-4 border-l-2 border-gray-300 dark:border-gray-600 italic text-gray-600 dark:text-gray-400 text-sm", children: _jsx("div", { className: "whitespace-pre-wrap", children: message.reasoning }) })] })), (() => {
                                    ...
                                    if ((trimmedContent.startsWith('{') || trimmedContent.startsWith('[')) &&
                                        ...
                                        try {
                                            ...
                                            return (_jsxs("div", { className: "my-2", children: [_jsxs("div", { className: "flex items-center gap-2 mb-2 text-sm text-gray-600 dark:text-gray-400", children: [_jsx("svg", { className: "w-4 h-4", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" }) }), _jsx("span", { className: "font-medium", children: "JSON Response" })] }), _jsx("div", { className: "bg-gray-800 dark:bg-gray-900 border border-gray-600/30 dark:border-gray-700 rounded-lg overflow-hidden", children: _jsx("pre", { className: "p-4 overflow-x-auto", children: _jsx("code", { className: "text-gray-100 dark:text-gray-200 text-sm font-mono block whitespace-pre", children: formatted }) }) })] }));
                                        }
                                        catch (e) {
                                    ...
                                    return message.type === 'assistant' ? (_jsx("div", { className: "prose prose-sm max-w-none dark:prose-invert prose-gray", children: _jsx(ReactMarkdown, { children: content }) })) : (_jsx("div", { className: "whitespace-pre-wrap", children: content }));
                                })()] })), _jsx("div", { className: `text-xs text-gray-500 dark:text-gray-400 mt-1 ${isGrouped ? 'opacity-0 group-hover:opacity-100' : ''}`, children: new Date(message.timestamp).toLocaleTimeString() })] })] })) }));
});
...
function ChatInterface({ selectedProject, selectedSession, ws, sendMessage, latestMessage, onFileOpen, onInputFocusChange, onSessionActive, onSessionInactive, onSessionProcessing, onSessionNotProcessing, processingSessions, onReplaceTemporarySession, onNavigateToSession, onShowSettings, autoExpandTools, showRawParameters, showThinking, autoScrollToBottom, sendByCtrlEnter, externalMessageUpdate, onTaskClick, onShowAllTasks }) {
    ...
    const { config: projectConfig, activeProvider: provider, claudeModel, cursorModel, codexModel, codexModelChoice, piModel, setActiveProvider: setProvider, setClaudeModel, setCursorModel, setCodexModel, setCodexModelChoice, setPiModel, isLoading: isConfigLoading } = useProjectModelConfig({
        projectId,
        autoSave: true,
        saveDelay: 500
    });
    ...
    const [dynamicCodexModels, setDynamicCodexModels] = useState([]);
    ...
    useEffect(() => {
        if (provider !== 'codex' || dynamicCodexModels.length > 0 || isLoadingCodexModels)
            return;
        const fetchCodexModels = async () => {
            ...
            try {
                ...
                if (data.success && data.models) {
                    const models = data.models.map((id) => ({ value: id, label: id }));
    ...
    const handleCustomCommand = useCallback(async (result, args) => {
        ...
        if (hasBashCommands) {
            const confirmed = window.confirm('This command contains bash commands that will be executed. Do you want to proceed?');
            if (!confirmed) {
                setChatMessages(prev => [...prev, {
                        role: 'assistant',
            ...
            switch (latestMessage.type) {
                case 'session-created':
                    ...
                    if (latestMessage.sessionId && !currentSessionId) {
                        ...
                        setPendingPermissionRequests(prev => prev.map(req => (req.sessionId ? req : { ...req, sessionId: latestMessage.sessionId })));
                    }
                    break;
                case 'token-budget':
    ...
    useEffect(() => {
        ...
        if (lastAtIndex !== -1) {
            ...
            if (!textAfterAt.includes(' ')) {
                ...
                const filtered = fileList.filter(file => file.name.toLowerCase().includes(textAfterAt.toLowerCase()) ||
                    file.path.toLowerCase().includes(textAfterAt.toLowerCase())).slice(0, 10); // Limit to 10 results
    ...
    const renderInputWithMentions = useCallback((text) => {
        ...
        return parts.map((part, index) => (fileMentionSet.has(part) ? (_jsx("span", { className: "bg-blue-200/70 -ml-0.5 dark:bg-blue-300/40 px-0.5 rounded-md box-decoration-clone text-transparent", children: part }, `mention-${index}`)) : (_jsx("span", { children: part }, `text-${index}`))));
    ...
    if (!selectedProject) {
        return (_jsx("div", { className: "flex items-center justify-center h-full", children: _jsx("div", { className: "text-center text-gray-500 dark:text-gray-400", children: _jsx("p", { children: "Select a project to start chatting with Claude" }) }) }));
    }
    return (_jsxs(_Fragment, { children: [_jsx("style", { children: `
          details[open] .details-chevron {
            transform: rotate(180deg);
          }
        ` }), _jsxs("div", { className: "h-full flex flex-col", children: [_jsxs("div", { ref: scrollContainerRef, onWheel: handleScroll, onTouchMove: handleScroll, className: "flex-1 overflow-y-auto overflow-x-hidden px-0 py-3 sm:p-4 space-y-3 sm:space-y-4 relative", children: [isLoadingSessionMessages && chatMessages.length === 0 ? (_jsx("div", { className: "text-center text-gray-500 dark:text-gray-400 mt-8", children: _jsxs("div", { className: "flex items-center justify-center space-x-2", children: [_jsx("div", { className: "animate-spin rounded-full h-4 w-4 border-b-2 border-gray-400" }), _jsx("p", { children: t('session.loading.sessionMessages') })] }) })) : chatMessages.length === 0 ? (_jsxs("div", { className: "flex items-center justify-center h-full", children: [!selectedSession && !currentSessionId && (_jsxs("div", { className: "text-center px-6 sm:px-4 py-8", children: [_jsx("h2", { className: "text-2xl font-bold text-gray-900 dark:text-white mb-3", children: t('providerSelection.title') }), _jsx("p", { className: "text-gray-600 dark:text-gray-400 mb-8", children: t('providerSelection.description') }), _jsxs("div", { className: "flex flex-col sm:flex-row gap-4 justify-center items-center mb-8", children: [_jsxs("button", { onClick: () => {
                                                            setProvider('claude');
                                                            ...
                                                            : 'border-gray-200 dark:border-gray-700 hover:border-blue-400'}`, children: [_jsxs("div", { className: "flex flex-col items-center justify-center h-full gap-3", children: [_jsx(ClaudeLogo, { className: "w-10 h-10" }), _jsxs("div", { children: [_jsx("p", { className: "font-semibold text-gray-900 dark:text-white", children: "Claude Code" }), _jsx("p", { className: "text-xs text-gray-500 dark:text-gray-400", children: t('providerSelection.providerInfo.anthropic') })] })] }), provider === 'claude' && (_jsx("div", { className: "absolute top-2 right-2", children: _jsx("div", { className: "w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center", children: _jsx("svg", { className: "w-3 h-3 text-white", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 3, d: "M5 13l4 4L19 7" }) }) }) }))] }), _jsxs("button", { onClick: () => {
                                                            setProvider('cursor');
                                                            // Focus input after selection
                                                            setTimeout(() => textareaRef.current?.focus(), 100);
                                                            ...
                                                            : 'border-gray-200 dark:border-gray-700 hover:border-purple-400'}`, children: [_jsxs("div", { className: "flex flex-col items-center justify-center h-full gap-3", children: [_jsx(CursorLogo, { className: "w-10 h-10" }), _jsxs("div", { children: [_jsx("p", { className: "font-semibold text-gray-900 dark:text-white", children: "Cursor" }), _jsx("p", { className: "text-xs text-gray-500 dark:text-gray-400", children: t('providerSelection.providerInfo.cursorEditor') })] })] }), provider === 'cursor' && (_jsx("div", { className: "absolute top-2 right-2", children: _jsx("div", { className: "w-5 h-5 bg-purple-500 rounded-full flex items-center justify-center", children: _jsx("svg", { className: "w-3 h-3 text-white", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 3, d: "M5 13l4 4L19 7" }) }) }) }))] }), _jsxs("button", { onClick: () => {
                                                            setProvider('codex');
                                                            // Focus input after selection
                                                            setTimeout(() => textareaRef.current?.focus(), 100);
                                                            ...
                                                            : 'border-gray-200 dark:border-gray-700 hover:border-gray-500 dark:hover:border-gray-400'}`, children: [_jsxs("div", { className: "flex flex-col items-center justify-center h-full gap-3", children: [_jsx(CodexLogo, { className: "w-10 h-10" }), _jsxs("div", { children: [_jsx("p", { className: "font-semibold text-gray-900 dark:text-white", children: "Codex" }), _jsx("p", { className: "text-xs text-gray-500 dark:text-gray-400", children: t('providerSelection.providerInfo.openai') })] })] }), provider === 'codex' && (_jsx("div", { className: "absolute top-2 right-2", children: _jsx("div", { className: "w-5 h-5 bg-gray-800 dark:bg-gray-300 rounded-full flex items-center justify-center", children: _jsx("svg", { className: "w-3 h-3 text-white dark:text-gray-800", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 3, d: "M5 13l4 4L19 7" }) }) }) }))] }), _jsxs("button", { onClick: () => {
                                                            setProvider('pi');
                                                            setTimeout(() => textareaRef.current?.focus(), 100);
                                                        }, className: `group relative w-64 h-32 bg-white dark:bg-gray-800 rounded-xl border-2 transition-all duration-200 hover:scale-105 hover:shadow-xl ${provider === 'pi'
                                                            ? 'border-amber-500 shadow-lg ring-2 ring-amber-500/20'
                                                            : 'border-gray-200 dark:border-gray-700 hover:border-amber-400'}`, children: [_jsxs("div", { className: "flex flex-col items-center justify-center h-full gap-3", children: [_jsx(PiLogo, { className: "w-10 h-10" }), _jsxs("div", { children: [_jsx("p", { className: "font-semibold text-gray-900 dark:text-white", children: "Pi" }), _jsx("p", { className: "text-xs text-gray-500 dark:text-gray-400", children: "pi-coding-agent" })] })] }), provider === 'pi' && (_jsx("div", { className: "absolute top-2 right-2", children: _jsx("div", { className: "w-5 h-5 bg-amber-500 rounded-full flex items-center justify-center", children: _jsx("svg", { className: "w-3 h-3 text-white", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 3, d: "M5 13l4 4L19 7" }) }) }) }))] })] }), _jsxs("div", { className: `mb-6 transition-opacity duration-200 ${provider ? 'opacity-100' : 'opacity-0 pointer-events-none'}`, children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2", children: t('providerSelection.selectModel') }), provider === 'claude' ? (_jsx("select", { value: claudeModel, onChange: (e) => {
                                                            setClaudeModel(e.target.value);
                                                        }, className: "pl-4 pr-10 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 min-w-[140px]", children: CLAUDE_MODELS.OPTIONS.map(({ value, label }) => (_jsx("option", { value: value, children: label }, value))) })) : provider === 'codex' ? (_jsxs("div", { className: "flex flex-col sm:flex-row gap-2", children: [_jsx("select", { value: codexModelChoice, onChange: (e) => {
                                                                    const choice = e.target.value;
                                                                    setCodexModelChoice(choice);
                                                                    if (choice !== CODEX_CUSTOM_MODEL) {
                                                                        setCodexModel(choice);
                                                                    }
                                                                }, className: "pl-4 pr-10 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-gray-500 min-w-[140px]", disabled: isLoadingCodexModels, children: isLoadingCodexModels ? (_jsx("option", { children: "Loading models..." })) : dynamicCodexModels.length > 0 ? (_jsxs(_Fragment, { children: [dynamicCodexModels.map(({ value, label }) => (_jsx("option", { value: value, children: label }, value))), _jsx("option", { value: CODEX_CUSTOM_MODEL, children: "Custom\u2026" })] })) : (_jsxs(_Fragment, { children: [CODEX_MODELS.OPTIONS.map(({ value, label }) => (_jsx("option", { value: value, children: label }, value))), _jsx("option", { value: CODEX_CUSTOM_MODEL, children: "Custom\u2026" })] })) }), codexModelChoice === CODEX_CUSTOM_MODEL && (_jsx("input", { value: codexModel, onChange: (e) => {
                                                                ...
                                                                }, placeholder: "model id, e.g. gpt-4.1", className: "px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-gray-500 min-w-[180px]" }))] })) : provider === 'pi' ? (_jsxs("div", { className: "flex flex-col sm:flex-row gap-2", children: [_jsx("input", { value: piProvider, onChange: (e) => {
                                                                    setPiProvider(e.target.value);
                                                                    localStorage.setItem('pi-provider', e.target.value);
                                                                }, placeholder: "provider (optional), e.g. openai", list: "pi-provider-options", className: "px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 min-w-[140px]" }), _jsx("select", { value: piModel, onChange: (e) => {
                                                                    setPiModel(e.target.value);
                                                                }, disabled: piModelOptions.length === 0, className: "px-3 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 min-w-[180px]", children: piModelOptions.length === 0 ? (_jsx("option", { value: "", children: "No Pi models configured" })) : (piModelOptions.map((modelId) => (_jsx("option", { value: modelId, children: modelId }, modelId)))) }), _jsx("datalist", { id: "pi-provider-options", children: piProviders.map((providerId) => (_jsx("option", { value: providerId }, providerId))) })] })) : (_jsx("select", { value: cursorModel, onChange: (e) => {
                                                            setCursorModel(e.target.value);
                                                        }, className: "pl-4 pr-10 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 min-w-[140px]", disabled: provider !== 'cursor', children: CURSOR_MODELS.OPTIONS.map(({ value, label }) => (_jsx("option", { value: value, children: label }, value))) }))] }), _jsx("p", { className: "text-sm text-gray-500 dark:text-gray-400", children: provider === 'claude'
                                                    ...
                                                    : provider === 'cursor'
                                                        ...
                                                        : provider === 'codex'
                                                            ...
                                                            : provider === 'pi'
                                                                ...
                                                                : t('providerSelection.readyPrompt.default') }), provider && tasksEnabled && isTaskMasterInstalled && (_jsx("div", { className: "mt-4 px-4 sm:px-0", children: _jsx(NextTaskBanner, { onStartTask: () => setInput('Start the next task'), onShowAllTasks: onShowAllTasks }) }))] })), selectedSession && (_jsxs("div", { className: "text-center text-gray-500 dark:text-gray-400 px-6 sm:px-4", children: [_jsx("p", { className: "font-bold text-lg sm:text-xl mb-3", children: t('session.continue.title') }), _jsx("p", { className: "text-sm sm:text-base leading-relaxed", children: t('session.continue.description') }), tasksEnabled && isTaskMasterInstalled && (_jsx("div", { className: "mt-4 px-4 sm:px-0", children: _jsx(NextTaskBanner, { onStartTask: () => setInput('Start the next task'), onShowAllTasks: onShowAllTasks }) }))] }))] })) : (_jsxs(_Fragment, { children: [isLoadingMoreMessages && (_jsx("div", { className: "text-center text-gray-500 dark:text-gray-400 py-3", children: _jsxs("div", { className: "flex items-center justify-center space-x-2", children: [_jsx("div", { className: "animate-spin rounded-full h-4 w-4 border-b-2 border-gray-400" }), _jsx("p", { className: "text-sm", children: t('session.loading.olderMessages') })] }) })), hasMoreMessages && !isLoadingMoreMessages && (_jsx("div", { className: "text-center text-gray-500 dark:text-gray-400 text-sm py-2 border-b border-gray-200 dark:border-gray-700", children: totalMessages > 0 && (_jsxs("span", { children: [t('session.messages.showingOf', { shown: sessionMessages.length, total: totalMessages }), " \u2022", _jsx("span", { className: "text-xs", children: t('session.messages.scrollToLoad') })] })) })), !hasMoreMessages && chatMessages.length > visibleMessageCount && (_jsxs("div", { className: "text-center text-gray-500 dark:text-gray-400 text-sm py-2 border-b border-gray-200 dark:border-gray-700", children: [t('session.messages.showingLast', { count: visibleMessageCount, total: chatMessages.length }), " \u2022", _jsx("button", { className: "ml-1 text-blue-600 hover:text-blue-700 underline", onClick: loadEarlierMessages, children: t('session.messages.loadEarlier') })] })), visibleMessages.map((message, index) => {
                                        const prevMessage = index > 0 ? visibleMessages[index - 1] : null;
                                        return (_jsx(MessageComponent, { message: message, index: index, prevMessage: prevMessage, createDiff: createDiff, onFileOpen: onFileOpen, onShowSettings: onShowSettings, onGrantToolPermission: handleGrantToolPermission, autoExpandTools: autoExpandTools, showRawParameters: showRawParameters, showThinking: showThinking, selectedProject: selectedProject, provider: provider }, index));
                                    })] })), isLoading && (_jsx("div", { className: "chat-message assistant", children: _jsxs("div", { className: "w-full", children: [_jsxs("div", { className: "flex items-center space-x-3 mb-2", children: [_jsx("div", { className: "w-8 h-8 rounded-full flex items-center justify-center text-white text-sm flex-shrink-0 p-1 bg-transparent", children: (localStorage.getItem('selected-provider') || 'claude') === 'cursor' ? (_jsx(CursorLogo, { className: "w-full h-full" })) : (localStorage.getItem('selected-provider') || 'claude') === 'codex' ? (_jsx(CodexLogo, { className: "w-full h-full" })) : (localStorage.getItem('selected-provider') || 'claude') === 'pi' ? (_jsx(PiLogo, { className: "w-full h-full" })) : (_jsx(ClaudeLogo, { className: "w-full h-full" })) }), _jsx("div", { className: "text-sm font-medium text-gray-900 dark:text-white", children: (localStorage.getItem('selected-provider') || 'claude') === 'cursor' ? 'Cursor' : (localStorage.getItem('selected-provider') || 'claude') === 'codex' ? 'Codex' : (localStorage.getItem('selected-provider') || 'claude') === 'pi' ? 'Pi' : 'Claude' })] }), _jsx("div", { className: "w-full text-sm text-gray-500 dark:text-gray-400 pl-3 sm:pl-0", children: _jsxs("div", { className: "flex items-center space-x-1", children: [_jsx("div", { className: "animate-pulse", children: "\u25CF" }), _jsx("div", { className: "animate-pulse", style: { animationDelay: '0.2s' }, children: "\u25CF" }), _jsx("div", { className: "animate-pulse", style: { animationDelay: '0.4s' }, children: "\u25CF" }), _jsx("span", { className: "ml-2", children: "Thinking..." })] }) })] }) })), _jsx("div", { ref: messagesEndRef })] }), _jsxs("div", { className: `p-2 sm:p-4 md:p-4 flex-shrink-0 ${isInputFocused ? 'pb-2 sm:pb-4 md:pb-6' : 'pb-2 sm:pb-4 md:pb-6'}`, children: [_jsx("div", { className: "flex-1", children: _jsx(ClaudeStatus, { status: claudeStatus, isLoading: isLoading, onAbort: handleAbortSession, provider: provider, showThinking: showThinking }) }), _jsxs("div", { ref: inputContainerRef, className: "max-w-4xl mx-auto mb-3", children: [pendingPermissionRequests.length > 0 && (
                                    ...
                                    _jsx("div", { className: "mb-3 space-y-2", children: pendingPermissionRequests.map((request) => {
                                            ...
                                            const matchingRequestIds = permissionEntry
                                                ...
                                                : [request.requestId];
                                            return (_jsxs("div", { className: "rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-3 shadow-sm", children: [_jsxs("div", { className: "flex flex-wrap items-start justify-between gap-3", children: [_jsxs("div", { children: [_jsx("div", { className: "text-sm font-semibold text-amber-900 dark:text-amber-100", children: "Permission required" }), _jsxs("div", { className: "text-xs text-amber-800 dark:text-amber-200", children: ["Tool: ", _jsx("span", { className: "font-mono", children: request.toolName })] })] }), permissionEntry && (_jsxs("div", { className: "text-xs text-amber-700 dark:text-amber-300", children: ["Allow rule: ", _jsx("span", { className: "font-mono", children: permissionEntry })] }))] }), rawInput && (_jsxs("details", { className: "mt-2", children: [_jsx("summary", { className: "cursor-pointer text-xs text-amber-800 dark:text-amber-200 hover:text-amber-900 dark:hover:text-amber-100", children: "View tool input" }), _jsx("pre", { className: "mt-2 max-h-40 overflow-auto rounded-md bg-white/80 dark:bg-gray-900/60 border border-amber-200/60 dark:border-amber-800/60 p-2 text-xs text-amber-900 dark:text-amber-100 whitespace-pre-wrap", children: rawInput })] })), _jsxs("div", { className: "mt-3 flex flex-wrap gap-2", children: [_jsx("button", { type: "button", onClick: () => handlePermissionDecision(request.requestId, { allow: true }), className: "inline-flex items-center gap-2 rounded-md bg-amber-600 text-white text-xs font-medium px-3 py-1.5 hover:bg-amber-700 transition-colors", children: "Allow once" }), _jsx("button", { type: "button", onClick: () => {
                                                                    ...
                                                                    : 'border-gray-300 text-gray-400 cursor-not-allowed'}`, disabled: !permissionEntry, children: rememberLabel }), _jsx("button", { type: "button", onClick: () => handlePermissionDecision(request.requestId, { allow: false, message: 'User denied tool use' }), className: "inline-flex items-center gap-2 rounded-md text-xs font-medium px-3 py-1.5 border border-red-300 text-red-700 hover:bg-red-50 dark:border-red-800 dark:text-red-200 dark:hover:bg-red-900/30 transition-colors", children: "Deny" })] })] }, request.requestId));
                                        }) })), _jsxs("div", { className: "flex items-center justify-center gap-3", children: [_jsx("button", { type: "button", onClick: handleModeSwitch, className: `px-3 py-1.5 rounded-lg text-sm font-medium border transition-all duration-200 ${permissionMode === 'default'
                                                    ...
                                                    : permissionMode === 'acceptEdits'
                                                        ...
                                                        : permissionMode === 'bypassPermissions'
                                                            ...
                                                            : 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/30'}`, title: t('input.clickToChangeMode'), children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: `w-2 h-2 rounded-full ${permissionMode === 'default'
                                                                ? 'bg-gray-500'
                                                                : permissionMode === 'acceptEdits'
                                                                    ? 'bg-green-500'
                                                                    : permissionMode === 'bypassPermissions'
                                                                        ? 'bg-orange-500'
                                                                        : 'bg-blue-500'}` }), _jsxs("span", { children: [permissionMode === 'default' && t('codex.modes.default'), permissionMode === 'acceptEdits' && t('codex.modes.acceptEdits'), permissionMode === 'bypassPermissions' && t('codex.modes.bypassPermissions'), permissionMode === 'plan' && t('codex.modes.plan')] })] }) }), provider === 'claude' && (_jsx(ThinkingModeSelector, { selectedMode: thinkingMode, onModeChange: setThinkingMode, className: "" })), _jsx(TokenUsagePie, { used: tokenBudget?.used || 0, total: tokenBudget?.total || parseInt(import.meta.env.VITE_CONTEXT_WINDOW) || 160000 }), _jsxs("button", { type: "button", onClick: () => {
                                                ...
                                                }, className: "relative w-8 h-8 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 rounded-full flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:ring-offset-gray-800", title: t('input.showAllCommands'), children: [_jsx("svg", { className: "w-5 h-5", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" }) }), slashCommands.length > 0 && (_jsx("span", { className: "absolute -top-1 -right-1 bg-blue-600 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center", style: { fontSize: '10px' }, children: slashCommands.length }))] }), input.trim() && (_jsx("button", { type: "button", onClick: (e) => {
                                                    e.preventDefault();
                                                    e.stopPropagation();
                                                    setInput('');
                                                    if (textareaRef.current) {
                                                    ...
                                                    setIsTextareaExpanded(false);
                                                }, className: "w-8 h-8 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-full flex items-center justify-center transition-all duration-200 group shadow-sm", title: "Clear input", children: _jsx("svg", { className: "w-4 h-4 text-gray-600 dark:text-gray-300 group-hover:text-gray-800 dark:group-hover:text-gray-100 transition-colors", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M6 18L18 6M6 6l12 12" }) }) })), isUserScrolledUp && chatMessages.length > 0 && (_jsx("button", { onClick: scrollToBottom, className: "w-8 h-8 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg flex items-center justify-center transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:ring-offset-gray-800", title: "Scroll to bottom", children: _jsx("svg", { className: "w-4 h-4", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M19 14l-7 7m0 0l-7-7m7 7V3" }) }) }))] })] }), _jsxs("form", { onSubmit: handleSubmit, className: "relative max-w-4xl mx-auto", children: [isDragActive && (_jsx("div", { className: "absolute inset-0 bg-blue-500/20 border-2 border-dashed border-blue-500 rounded-lg flex items-center justify-center z-50", children: _jsxs("div", { className: "bg-white dark:bg-gray-800 rounded-lg p-4 shadow-lg", children: [_jsx("svg", { className: "w-8 h-8 text-blue-500 mx-auto mb-2", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" }) }), _jsx("p", { className: "text-sm font-medium", children: "Drop images here" })] }) })), attachedImages.length > 0 && (_jsx("div", { className: "mb-2 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg", children: _jsx("div", { className: "flex flex-wrap gap-2", children: attachedImages.map((file, index) => (_jsx(ImageAttachment, { file: file, onRemove: () => {
                                                    setAttachedImages(prev => prev.filter((_, i) => i !== index));
                                                }, uploadProgress: uploadingImages.get(file.name), error: imageErrors.get(file.name) }, index))) }) })), showFileDropdown && filteredFiles.length > 0 && (_jsx("div", { className: "absolute bottom-full left-0 right-0 mb-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg max-h-48 overflow-y-auto z-50 backdrop-blur-sm", children: filteredFiles.map((file, index) => (_jsxs("div", { className: `px-4 py-3 cursor-pointer border-b border-gray-100 dark:border-gray-700 last:border-b-0 touch-manipulation ${index === selectedFileIndex
                                                ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300'
                                                : 'hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`, onMouseDown: (e) => {
                                                ...
                                                e.stopPropagation();
                                                selectFile(file);
                                            }, children: [_jsx("div", { className: "font-medium text-sm", children: file.name }), _jsx("div", { className: "text-xs text-gray-500 dark:text-gray-400 font-mono", children: file.path })] }, file.path))) })), _jsx(CommandMenu, { commands: filteredCommands, selectedIndex: selectedCommandIndex, onSelect: handleCommandSelect, onClose: () => {
                                            setShowCommandMenu(false);
                                            setSlashPosition(-1);
                                            setCommandQuery('');
                                            setSelectedCommandIndex(-1);
                                            ...
                                            bottom: textareaRef.current
                                                ...
                                                : 90
                                        }, isOpen: showCommandMenu, frequentCommands: commandQuery ? [] : frequentCommands }), _jsxs("div", { ...getRootProps(), className: `relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-600 focus-within:ring-2 focus-within:ring-blue-500 dark:focus-within:ring-blue-500 focus-within:border-blue-500 transition-all duration-200 overflow-hidden ${isTextareaExpanded ? 'chat-input-expanded' : ''}`, children: [_jsx("input", { ...getInputProps() }), _jsx("div", { ref: inputHighlightRef, "aria-hidden": "true", className: "absolute inset-0 pointer-events-none overflow-hidden rounded-2xl", children: _jsx("div", { className: "chat-input-placeholder block w-full pl-12 pr-20 sm:pr-40 py-1.5 sm:py-4 text-transparent text-base leading-6 whitespace-pre-wrap break-words", children: renderInputWithMentions(input) }) }), _jsxs("div", { className: "relative z-10", children: [_jsx("textarea", { ref: textareaRef, value: input, onChange: handleInputChange, onClick: handleTextareaClick, onKeyDown: handleKeyDown, onPaste: handlePaste, onScroll: (e) => syncInputOverlayScroll(e.target), onFocus: () => setIsInputFocused(true), onBlur: () => setIsInputFocused(false), onInput: (e) => {
                                                            ...
                                                            setIsTextareaExpanded(isExpanded);
                                                        }, placeholder: t('input.placeholder', { provider: provider === 'cursor' ? t('messageTypes.cursor') : provider === 'codex' ? t('messageTypes.codex') : provider === 'pi' ? t('messageTypes.pi') : t('messageTypes.claude') }), disabled: isLoading, className: "chat-input-placeholder block w-full pl-12 pr-20 sm:pr-40 py-1.5 sm:py-4 bg-transparent rounded-2xl focus:outline-none text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 disabled:opacity-50 resize-none min-h-[50px] sm:min-h-[80px] max-h-[40vh] sm:max-h-[300px] overflow-y-auto text-base leading-6 transition-all duration-200", style: { height: '50px' } }), _jsx("button", { type: "button", onClick: open, className: "absolute left-2 top-1/2 transform -translate-y-1/2 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors", title: t('input.attachImages'), children: _jsx("svg", { className: "w-5 h-5 text-gray-500", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" }) }) }), _jsx("div", { className: "absolute right-16 sm:right-16 top-1/2 transform -translate-y-1/2", style: { display: 'none' }, children: _jsx(MicButton, { onTranscript: handleTranscript, className: "w-10 h-10 sm:w-10 sm:h-10" }) }), _jsx("button", { type: "submit", disabled: !input.trim() || isLoading, onMouseDown: (e) => {
                                                            e.preventDefault();
                                                            handleSubmit(e);
                                                        }, onTouchStart: (e) => {
                                                            e.preventDefault();
                                                            handleSubmit(e);
                                                        }, className: "absolute right-2 top-1/2 transform -translate-y-1/2 w-12 h-12 sm:w-12 sm:h-12 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed rounded-full flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:ring-offset-gray-800", children: _jsx("svg", { className: "w-4 h-4 sm:w-5 sm:h-5 text-white transform rotate-90", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 19l9 2-9-18-9 18 9-2zm0 0v-8" }) }) }), _jsx("div", { className: `absolute bottom-1 left-12 right-14 sm:right-40 text-xs text-gray-400 dark:text-gray-500 pointer-events-none hidden sm:block transition-opacity duration-200 ${input.trim() ? 'opacity-0' : 'opacity-100'}`, children: sendByCtrlEnter
