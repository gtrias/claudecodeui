...
const Tooltip = ({ children, content, position = 'top', className = '', delay = 500 }) => {
    ...
    return (_jsxs("div", { className: "relative inline-block", onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, children: [children, isVisible && (_jsxs("div", { className: cn('absolute z-50 px-2 py-1 text-xs font-medium text-white bg-gray-900 dark:bg-gray-100 dark:text-gray-900 rounded shadow-lg whitespace-nowrap pointer-events-none', 'animate-in fade-in-0 zoom-in-95 duration-200', getPositionClasses(), className), children: [content, _jsx("div", { className: cn('absolute w-0 h-0 border-4 border-transparent', getArrowClasses()) })] }))] }));
};
