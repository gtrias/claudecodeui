...
import { X, Save, Download, Maximize2, Minimize2, Edit } from 'lucide-react';
import { api } from '../utils/api';
const InlineDiffView = ({ oldContent, newContent, fileName, isDarkMode, onEdit }) => {
    const changes = useMemo(() => diffLines(oldContent, newContent), [oldContent, newContent]);
    const renderLine = (line, type, index) => {
        ...
        return (_jsx("div", { className: `font-mono text-sm px-4 py-0.5 ${bgColor} ${textColor} whitespace-pre-wrap break-words`, children: line || ' ' }, index));
    };
    ...
    return (_jsxs("div", { className: "flex flex-col h-full", children: [_jsxs("div", { className: `flex items-center justify-between px-4 py-2 border-b ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200'}`, children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("span", { className: "font-medium text-sm", children: fileName }), _jsxs("span", { className: `text-xs px-2 py-1 rounded ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-700'}`, children: [stats.additions, " additions, ", stats.deletions, " deletions"] })] }), _jsxs("button", { onClick: onEdit, className: `flex items-center gap-1 px-3 py-1 text-sm rounded transition-colors ${isDarkMode
                            ? 'bg-blue-600 text-white hover:bg-blue-700'
                            : 'bg-blue-500 text-white hover:bg-blue-600'}`, children: [_jsx(Edit, { className: "w-4 h-4" }), "Edit"] })] }), _jsx("div", { className: "flex-1 overflow-auto", children: changes.length === 0 ? (_jsx("div", { className: "flex items-center justify-center h-full text-gray-500", children: "No changes" })) : (changes.map((part, partIndex) => {
                    if (part.removed) {
                        return part.value.split('\n').map((line, lineIndex) => renderLine(line, 'removed', `${partIndex}-removed-${lineIndex}`));
                    }
                    if (part.added) {
                        return part.value.split('\n').map((line, lineIndex) => renderLine(line, 'added', `${partIndex}-added-${lineIndex}`));
                    }
                    return part.value.split('\n').map((line, lineIndex) => renderLine(line, 'context', `${partIndex}-context-${lineIndex}`));
                })) })] }));
};
function CodeEditor({ file, onClose, projectPath, isSidebar = false, isExpanded = false, onToggleExpand = null }) {
    ...
    const languageExtensions = useMemo(() => getLanguageExtension(file?.name ?? ''), [file?.name]);
    // Load file content
    useEffect(() => {
    ...
    if (loading) {
        return (_jsxs(_Fragment, { children: [_jsx("style", { children: `
          ...
          ` }), isSidebar ? (_jsx("div", { className: "w-full h-full flex items-center justify-center bg-background", children: _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("div", { className: "animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600" }), _jsx("span", { className: "text-gray-900 dark:text-white", children: t('loading', { fileName: file?.name || '...' }) })] }) })) : (_jsx("div", { className: "fixed inset-0 z-40 md:bg-black/50 md:flex md:items-center md:justify-center", children: _jsx("div", { className: "code-editor-loading w-full h-full md:rounded-lg md:w-auto md:h-auto p-8 flex items-center justify-center", children: _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("div", { className: "animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600" }), _jsx("span", { className: "text-gray-900 dark:text-white", children: t('loading', { fileName: file?.name || '...' }) })] }) }) }))] }));
    }
    return (_jsxs(_Fragment, { children: [_jsx("style", { children: `
        ...
        ` }), _jsx("div", { className: isSidebar ?
                    'w-full h-full flex flex-col' :
                    `fixed inset-0 z-40 ${
                    // Mobile: native fullscreen, Desktop: modal with backdrop
                    'md:bg-black/50 md:flex md:items-center md:justify-center md:p-4'} ${isFullscreen ? 'md:p-0' : ''}`, children: _jsxs("div", { className: isSidebar ?
                        'bg-background flex flex-col w-full h-full' :
                        `bg-background shadow-2xl flex flex-col ${
                        // Mobile: always fullscreen, Desktop: modal sizing
                        'w-full h-full md:rounded-lg md:shadow-2xl' +
                            (isFullscreen ? ' md:w-full md:h-full md:rounded-none' : ' md:w-full md:max-w-6xl md:h-[80vh] md:max-h-[80vh]')}`, children: [_jsxs("div", { className: "flex items-center justify-between p-4 border-b border-border flex-shrink-0 min-w-0", children: [_jsx("div", { className: "flex items-center gap-3 min-w-0 flex-1", children: _jsxs("div", { className: "min-w-0 flex-1", children: [_jsxs("div", { className: "flex items-center gap-2 min-w-0", children: [_jsx("h3", { className: "font-medium text-gray-900 dark:text-white truncate", children: file?.name || 'Unknown File' }), file?.diffInfo && (_jsx("button", { onClick: () => setShowDiff(!showDiff), className: "text-xs bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 px-2 py-1 rounded whitespace-nowrap hover:bg-blue-200 dark:hover:bg-blue-800 transition-colors", children: showDiff ? 'Editing' : 'Showing Changes' }))] }), _jsx("p", { className: "text-sm text-gray-500 dark:text-gray-400 truncate", children: file?.path || '' })] }) }), _jsxs("div", { className: "flex items-center gap-1 md:gap-2 flex-shrink-0", children: [_jsx("button", { onClick: handleDownload, className: "p-2 md:p-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0 flex items-center justify-center", title: t('actions.download'), children: _jsx(Download, { className: "w-5 h-5 md:w-4 md:h-4" }) }), _jsx("button", { onClick: handleSave, disabled: saving, className: `px-3 py-2 text-white rounded-md disabled:opacity-50 flex items-center gap-2 transition-colors min-h-[44px] md:min-h-0 ${saveSuccess
                                                ? 'bg-green-600 hover:bg-green-700'
                                                : 'bg-blue-600 hover:bg-blue-700'}`, children: saveSuccess ? (_jsxs(_Fragment, { children: [_jsx("svg", { className: "w-5 h-5 md:w-4 md:h-4", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M5 13l4 4L19 7" }) }), _jsx("span", { className: "hidden sm:inline", children: t('actions.saved') })] })) : (_jsxs(_Fragment, { children: [_jsx(Save, { className: "w-5 h-5 md:w-4 md:h-4" }), _jsx("span", { className: "hidden sm:inline", children: saving ? t('actions.saving') : t('actions.save') })] })) }), !isSidebar && (_jsx("button", { onClick: toggleFullscreen, className: "hidden md:flex p-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 items-center justify-center", title: isFullscreen ? t('actions.exitFullscreen') : t('actions.fullscreen'), children: isFullscreen ? _jsx(Minimize2, { className: "w-4 h-4" }) : _jsx(Maximize2, { className: "w-4 h-4" }) })), _jsx("button", { onClick: onClose, className: "p-2 md:p-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0 flex items-center justify-center", title: t('actions.close'), children: _jsx(X, { className: "w-6 h-6 md:w-4 md:h-4" }) })] })] }), _jsx("div", { className: "flex-1 overflow-hidden", children: showDiff && file?.diffInfo && file.diffInfo.old_string !== undefined ? (
                            // Show inline diff view
                            _jsx(InlineDiffView, { oldContent: file.diffInfo.old_string, newContent: file.diffInfo.new_string || content, fileName: file?.name || '', isDarkMode: isDarkMode, onEdit: () => setShowDiff(false) })) : (
                            // Show normal editor
                            _jsx(CodeMirror, { ref: editorRef, value: content, onChange: setContent, extensions: [
                                    ...languageExtensions,
                                    ...
                                    searchKeymap: true,
                                } })) }), _jsxs("div", { className: "flex items-center justify-between p-3 border-t border-border bg-muted flex-shrink-0", children: [_jsxs("div", { className: "flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400", children: [_jsxs("span", { children: [t('footer.lines'), " ", content.split('\n').length] }), _jsxs("span", { children: [t('footer.characters'), " ", content.length] })] }), _jsx("div", { className: "text-sm text-gray-500 dark:text-gray-400", children: t('footer.shortcuts') })] })] }) })] }));
}
