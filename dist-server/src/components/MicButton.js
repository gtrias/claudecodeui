...
export const MicButton = ({ onTranscript, className = '' }) => {
    ...
    const getButtonAppearance = () => {
        if (!isSupported) {
            return {
                icon: _jsx(Mic, { className: "w-5 h-5" }),
        ...
        switch (state) {
            case 'recording':
                return {
                    icon: _jsx(Mic, { className: "w-5 h-5 text-white" }),
                    className: 'bg-red-500 hover:bg-red-600 animate-pulse',
                    disabled: false
                };
            case 'transcribing':
                return {
                    icon: _jsx(Loader2, { className: "w-5 h-5 animate-spin" }),
                    className: 'bg-blue-500 hover:bg-blue-600',
                    disabled: true
                };
            case 'processing':
                return {
                    icon: _jsx(Brain, { className: "w-5 h-5 animate-pulse" }),
    ...
    return (_jsxs("div", { className: "relative", children: [_jsx("button", { type: "button", style: {
          ...
          hover:opacity-90
          ${className}
        `, onClick: handleClick, disabled: disabled, children: icon }), error && (_jsx("div", { className: "absolute top-full mt-2 left-1/2 transform -translate-x-1/2 \n                        bg-red-500 text-white text-xs px-2 py-1 rounded whitespace-nowrap z-10\n                        animate-fade-in", children: error })), state === 'recording' && (_jsx("div", { className: "absolute -inset-1 rounded-full border-2 border-red-500 animate-ping pointer-events-none" })), state === 'processing' && (_jsx("div", { className: "absolute -inset-1 rounded-full border-2 border-purple-500 animate-ping pointer-events-none" }))] }));
};
