import React, { useState, useEffect, useRef, useCallback, MouseEvent, TouchEvent, CSSProperties } from 'react';
import {
  ChevronLeft,
  ChevronRight,
  Maximize2,
...
type WhisperMode = 'default' | 'prompt' | 'vibe' | 'instructions' | 'architect';
...
const QuickSettingsPanel: React.FC<QuickSettingsPanelProps> = ({
  ...
  const [handlePosition, setHandlePosition] = useState(() => {
    ...
    if (saved) {
      try {
        const parsed: HandlePosition = JSON.parse(saved);
        return parsed.y ?? 50;
      } catch {
  ...
  const constraintsRef = useRef<PositionConstraints>({ min: 10, max: 90 }); // Percentage constraints
  ...
  const handleDragStart = useCallback((e: MouseEvent<HTMLButtonElement> | TouchEvent<HTMLButtonElement>) => {
    // Don't prevent default yet - we want to allow click if no drag happens
    e.stopPropagation();

    const clientY = e.type.includes('touch') 
      ? (e as TouchEvent<HTMLButtonElement>).touches[0].clientY 
      : (e as MouseEvent<HTMLButtonElement>).clientY;
  ...
  const handleWhisperModeChange = (mode: WhisperMode) => {
    setWhisperMode(mode);
  ...
  return (
    <>
      ...
      <button
        ...
        style={{ ...getPositionStyle(), touchAction: 'none', WebkitTouchCallout: 'none', WebkitUserSelect: 'none' } as CSSProperties}
