...
type WorkspaceType = 'existing' | 'new';
type TokenMode = 'stored' | 'new' | 'none';
...
const ProjectCreationWizard: React.FC<ProjectCreationWizardProps> = ({ onClose, onProjectCreated }) => {
  ...
  const [workspaceType, setWorkspaceType] = useState<WorkspaceType>('existing'); // 'existing' or 'new' - default to 'existing'

  // Form state
  const [workspacePath, setWorkspacePath] = useState('');
  const [githubUrl, setGithubUrl] = useState('');
  const [selectedGithubToken, setSelectedGithubToken] = useState('');
  const [tokenMode, setTokenMode] = useState<TokenMode>('stored'); // 'stored' | 'new' | 'none'
