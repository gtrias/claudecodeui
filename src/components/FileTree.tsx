...
function FileTree({ selectedProject }: FileTreeProps): JSX.Element {
  ...
  useEffect(() => {
    if (!searchQuery.trim()) {
      ...
      const expandMatches = (items: FileItem[]) => {
        items.forEach(item => {
  ...
  const renderDetailedView = (items: FileItem[], level = 0): JSX.Element[] => {
    return items.map((item) => (
      <div key={item.path} className="select-none">
        <div
          className={cn(
  ...
  const renderCompactView = (items: FileItem[], level = 0): JSX.Element[] => {
    return items.map((item) => (
      <div key={item.path} className="select-none">
        <div
          className={cn(
