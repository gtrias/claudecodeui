...
import { Task, TaskPriority } from './TaskCard';
...
type TemplateSelectorStep = 'select' | 'customize' | 'generate';
    ...
    <>
      ...
      {showTaskDetail && nextTask && (
        <TaskDetail
          ...
          onTaskClick={undefined} // Disable dependency navigation in NextTaskBanner for now
        />
      )}
    </>
  );
...
const TemplateSelector: React.FC<TemplateSelectorProps> = ({ currentProject, onClose, onTemplateApplied }) => {
  ...
  const [step, setStep] = useState<TemplateSelectorStep>('select');
