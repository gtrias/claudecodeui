...
interface CursorSession extends BaseSession {
  name?: string;
  createdAt: string;
  __provider: 'cursor';
}

interface CodexSession extends BaseSession {
  name?: string;
  summary?: string;
  createdAt?: string;
  __provider: 'codex';
...
interface PiSession extends BaseSession {
  name?: string;
  summary?: string;
  createdAt?: string;
  __provider: 'pi';
...
type Session = ClaudeSession | CursorSession | CodexSession | PiSession;
...
function Sidebar({
  ...
  const [additionalSessions, setAdditionalSessions] = useState<Record<string, ClaudeSession[]>>({});
  ...
  const handleSessionClick = (session: Session, projectName: string) => {
  ...
  const isProjectStarred = (projectName: string): boolean => {
    return starredProjects.has(projectName);
  };

  // Helper function to get all sessions for a project (initial + additional)
  const getAllSessions = (project: Project): Session[] => {
    // Combine Claude, Cursor, Codex, and Pi sessions; Sidebar will display icon per row
    const claudeSessions: ClaudeSession[] = [...(project.sessions || []), ...(additionalSessions[project.name] || [])].map(s => ({ ...s, __provider: 'claude' as const }));
    const cursorSessions: CursorSession[] = (project.cursorSessions || []).map(s => ({ ...s, __provider: 'cursor' as const }));
    const codexSessions: CodexSession[] = (project.codexSessions || []).map(s => ({ ...s, __provider: 'codex' as const }));
    const piSessions: PiSession[] = (project.piSessions || []).map(s => ({ ...s, __provider: 'pi' as const }));
    
    // Sort by most recent activity/date
    const normalizeDate = (s: Session): Date => {
      if (s.__provider === 'cursor') return new Date(s.createdAt);
      if (s.__provider === 'codex') return new Date((s as CodexSession).createdAt || s.lastActivity);
      if (s.__provider === 'pi') return new Date(s.lastActivity || (s as PiSession).createdAt || '');
  ...
  const saveProjectName = async (projectName: string) => {
    try {
      ...
      if (response.ok) {
        ...
        if ((window as any).refreshProjects) {
  ...
  const updateSessionSummary = async (projectName: string, sessionId: string, newSummary: string) => {
    try {
                      ...
                      {!initialSessionsLoaded.has(project.name) ? (
                        ...
                        getAllSessions(project).map((session) => {
                          ...
                          const getSessionDate = (): Date => {
                            if (isCursorSession) return new Date((session as CursorSession).createdAt);
                            if (isCodexSession) return new Date((session as CodexSession).createdAt || session.lastActivity);
                            if (isPiSession) return new Date(session.lastActivity || (session as PiSession).createdAt || '');
                          ...
                          const getSessionName = (): string => {
                            ...
                            return (session as ClaudeSession).summary || t('projects.newSession');
                          };
                          const sessionName = getSessionName();
                          const getSessionTime = (): string => {
                            if (isCursorSession) return (session as CursorSession).createdAt;
                            if (isCodexSession) return (session as CodexSession).createdAt || session.lastActivity;
                            if (isPiSession) return session.lastActivity || (session as PiSession).createdAt || '';
                                    ...
                                    {!isCodexSession && !isPiSession && (
                                      <button
                                        ...
                                        onClick={(e) => {
                                          ...
                                          setEditingSessionName((session as ClaudeSession).summary || t('projects.newSession'));
                                        }}
