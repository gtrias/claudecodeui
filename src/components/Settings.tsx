...
const Settings: FC<SettingsProps> = ({ isOpen, onClose, projects = [], initialTab = 'agents' }) => {
  ...
  const [projectSortOrder, setProjectSortOrder] = useState<'name' | 'date'>('name');

  const [mcpServers, setMcpServers] = useState<McpServer[]>([]);
  const [showMcpForm, setShowMcpForm] = useState<boolean>(false);
  ...
  const [mcpTestResults, setMcpTestResults] = useState<Record<string, TestResult>>({});
  const [mcpServerTools, setMcpServerTools] = useState<Record<string, ToolsResult>>({});
  ...
  const [selectedAgent, setSelectedAgent] = useState<'claude' | 'cursor' | 'codex' | 'pi'>('claude'); // 'claude', 'cursor', or 'codex'
  const [selectedCategory, setSelectedCategory] = useState<'account' | 'permissions' | 'mcp'>('account'); // 'account', 'permissions', or 'mcp'

  // Code Editor settings
  const [codeEditorTheme, setCodeEditorTheme] = useState<string>(() =>
    localStorage.getItem('codeEditorTheme') || 'dark'
  );
  const [codeEditorWordWrap, setCodeEditorWordWrap] = useState<boolean>(() =>
    localStorage.getItem('codeEditorWordWrap') === 'true'
  );
  const [codeEditorShowMinimap, setCodeEditorShowMinimap] = useState<boolean>(() =>
    localStorage.getItem('codeEditorShowMinimap') !== 'false' // Default true
  );
  const [codeEditorLineNumbers, setCodeEditorLineNumbers] = useState<boolean>(() =>
    localStorage.getItem('codeEditorLineNumbers') !== 'false' // Default true
  );
  const [codeEditorFontSize, setCodeEditorFontSize] = useState<string>(() =>
  ...
  const [editingCodexMcpServer, setEditingCodexMcpServer] = useState<any | null>(null);
  const [codexMcpLoading, setCodexMcpLoading] = useState<boolean>(false);
  ...
  const saveMcpServer = async (serverData: McpFormData): Promise<boolean> => {
    try {
      if (editingMcpServer) {
  ...
  const deleteMcpServer = async (serverId: string, scope: string = 'user'): Promise<boolean> => {
    try {
  ...
  const testMcpServer = async (serverId: string, scope: string = 'user'): Promise<any> => {
    try {
  ...
  const discoverMcpTools = async (serverId: string, scope: string = 'user'): Promise<any> => {
    try {
  ...
  const saveCodexMcpServer = async (serverData: CodexMcpFormData): Promise<boolean> => {
    try {
  ...
  const deleteCodexMcpServer = async (serverId: string): Promise<boolean> => {
    try {
  ...
  const openCodexMcpForm = (server: any | null = null): void => {
    if (server) {
  ...
  const handleCodexMcpSubmit = async (e: React.FormEvent): Promise<void> => {
    e.preventDefault();
  ...
  const handleCodexMcpDelete = async (serverName: string): Promise<void> => {
  ...
  const addAllowedTool = (tool: string): void => {
  ...
  const removeAllowedTool = (tool: string): void => {
    setAllowedTools(allowedTools.filter(t => t !== tool));
  };

  const addDisallowedTool = (tool: string): void => {
  ...
  const removeDisallowedTool = (tool: string): void => {
  ...
  const openMcpForm = (server: McpServer | null = null): void => {
    if (server) {
  ...
  const handleMcpSubmit = async (e: React.FormEvent): Promise<void> => {
    e.preventDefault();
  ...
  const handleMcpDelete = async (serverId: string, scope: string): Promise<void> => {
    if (confirm('Are you sure you want to delete this MCP server?')) {
      try {
  ...
  const handleMcpTest = async (serverId: string, scope: string): Promise<void> => {
    try {
  ...
  const handleMcpToolsDiscovery = async (serverId: string, scope: string): Promise<void> => {
    try {
  ...
  const updateMcpConfig = (key: string, value: any): void => {
    setMcpFormData(prev => ({
      ...prev,
      config: {
        ...prev.config,
  ...
  const getTransportIcon = (type: string): JSX.Element => {
    switch (type) {
