...
interface TaskMasterSetupWizardProps {
  isOpen?: boolean;
  onClose?: () => void;
  onComplete?: () => void;
...
const TaskMasterSetupWizard: React.FC<TaskMasterSetupWizardProps> = ({ 
  ...
  const handleNext = async () => {
    ...
    try {
      if (currentStep === 1) {
        ...
        setLoading(true);
        try {
          const mcpStatus: MCPStatus = await api.get('/mcp-utils/taskmaster-server');
          setSetupData(prev => ({
            ...prev,
  ...
  const completeSetup = async () => {
    ...
    try {
      ...
      setError((err as Error).message || 'Failed to complete TaskMaster setup');
    } finally {
      setLoading(false);
    }
  };
